...
\begin{frame}[shrink=20]{Last Time: A Deliberately Simple Micromouse Model}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      ...
      \lectureimage[2.45in]{micromouse-simple-grid-model.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{A Search Algorithm Can Only Use the Actions We Define}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \lectureimage[2.55in]{orthogonal-action-set-limits.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Micromouse Risked Becoming a ``Solved'' Problem}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \lectureimage[2.48in]{micromouse-apparent-stagnation.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Mitee Mouse III Changed the Action Space}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      ...
      \lectureimage[2.48in]{mitee3-diagonal-running.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Orientation Now Becomes Part of the State}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.41\textwidth}
      % Search direction: same maze cell shown with two robot orientations producing different legal next movements.
      \lectureimage[2.55in]{orientation-changes-actions.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{A Car Cannot Simply ``Move Left''}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      % Search direction: top-down car turning left along a curved arc compared with an impossible sideways grid translation.
      \lectureimage[2.55in]{car-left-action-is-arc.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Kinematic Constraints Restrict Reachable States}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      % Search direction: kinematic bicycle-model diagram with x, y, heading theta, wheelbase, and steering angle.
      \lectureimage[2.42in]{kinematic-bicycle-model.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Velocity Can Change Which Actions Are Feasible}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      % Search direction: two identical cars at different speeds approaching a stopping point, with dramatically different stopping distances.
      \lectureimage[2.50in]{velocity-dependent-stopping.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Beyond Kinematics: Dynamics and External Conditions}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      % Search direction: same car cornering on dry, wet, and icy pavement with increasing slip trajectories.
      \lectureimage[2.42in]{road-friction-changes-trajectory.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Micromouse Faced the Same Physical Limits}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.39\textwidth}
      % Search direction: high-speed Micromouse cornering with suction fan / traction concept, not a generic mouse photo.
      \lectureimage[2.45in]{micromouse-traction-and-fan.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Shortest Path Is Not Necessarily Fastest Path}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      ...
      \lectureimage[2.50in]{micromouse-shortest-vs-fastest.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Depth and Cost Can Disagree}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      % Search direction: small weighted graph contrasting a two-edge expensive path and a three-edge cheap path.
      \lectureimage[2.48in]{depth-versus-path-cost.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Edsger W. Dijkstra}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.39\textwidth}
      ...
      \lectureimage[2.50in]{dijkstra-1959-paper.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Worked Example: Weighted Search Graph}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.48\textwidth}
      % Search direction: clean weighted directed graph with nodes S,A,B,C,D,G and the listed edge costs.
      \lectureimage[2.75in]{dijkstra-example-graph.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Dijkstra Step 0: Initialize the Frontier}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      % Search direction: Dijkstra example graph with S highlighted and all other nodes labeled infinity.
      \lectureimage[2.55in]{dijkstra-step0-initialize.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Dijkstra Step 1: Expand \texttt{S}}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.47\textwidth}
      % Search direction: weighted graph after expanding S, with A=2 and B=5 highlighted as frontier values.
      \lectureimage[1.65in]{dijkstra-step1-expand-s.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Same Puzzle, Different Objective}
  ...
  \begin{center}
    % Search direction: 8-puzzle showing two candidate solution sequences, one fewer moves but expensive tiles and one more moves but cheaper tiles.
    \lectureimage[1.70in]{weighted-sliding-tile-puzzle.png}
  \end{center}
...
\begin{frame}[shrink=20]{Robotics and Autonomous Navigation}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      % Search direction: warehouse or mobile robot occupancy grid with weighted terrain cells and a minimum-cost route.
      \lectureimage[2.45in]{robot-weighted-navigation.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{The Cost Contour Can Expand in the Wrong Direction}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      % Search direction: weighted grid with start left, goal right, and Dijkstra cost contours expanding heavily into a cheap but irrelevant southern region.
      \lectureimage[2.55in]{ucs-cheap-irrelevant-region.png}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Image Placeholder Index}
  \scriptsize
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{itemize}
        \item \texttt{micromouse-simple-grid-model.png}
        \item \texttt{orthogonal-action-set-limits.png}
        \item \texttt{micromouse-apparent-stagnation.png}
        \item \texttt{mitee3-diagonal-running.png}
        \item \texttt{orientation-changes-actions.png}
        \item \texttt{car-left-action-is-arc.png}
        \item \texttt{kinematic-bicycle-model.png}
        \item \texttt{velocity-dependent-stopping.png}
        \item \texttt{road-friction-changes-trajectory.png}
      \end{itemize}
    \end{column}

    \begin{column}{0.49\textwidth}
      \begin{itemize}
        \item \texttt{micromouse-traction-and-fan.png}
        \item \texttt{micromouse-shortest-vs-fastest.png}
        \item \texttt{depth-versus-path-cost.png}
        \item \texttt{dijkstra-1959-paper.png}
        \item \texttt{dijkstra-example-graph.png}
        \item \texttt{dijkstra-step0-initialize.png}
        \item \texttt{dijkstra-step1-expand-s.png}
        \item \texttt{weighted-sliding-tile-puzzle.png}
        \item \texttt{robot-weighted-navigation.png}
        \item \texttt{ucs-cheap-irrelevant-region.png}
      \end{itemize}
    \end{column}
  \end{columns}
