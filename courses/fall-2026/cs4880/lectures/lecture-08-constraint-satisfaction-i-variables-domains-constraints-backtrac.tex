...
\begin{frame}[shrink=20]{Humans Reason With Constraints Constantly}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.39\textwidth}
      \lectureplaceholder[2.55in]{Split visual: calendar scheduling constraints and a seating arrangement with allowed/forbidden adjacencies.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Map Coloring as a Constraint Problem}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.43\textwidth}
      \lectureplaceholder[2.70in]{Outline map of Australia labeled WA, NT, SA, Q, NSW, V, T; no colors assigned yet.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Scaling Exposes the Real Problem}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.65in]{Map of the contiguous United States colored with three colors; emphasize many interacting adjacency constraints.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{The Declarative Perspective}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.39\textwidth}
      \lectureplaceholder[2.50in]{Diagram: problem specification (variables + domains + constraints) flows into a generic CSP solver, producing a satisfying assignment.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Australia as a CSP}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.39\textwidth}
      \lectureplaceholder[2.75in]{Australia map labeled by variables; annotate a few borders with inequality symbols.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Constraint Graphs}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      \lectureplaceholder[2.60in]{Constraint graph for Australia: WA, NT, SA, Q, NSW, V, T with edges representing adjacency.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{A Solution Is Not a Path}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      \lectureplaceholder[2.55in]{Completed Australia coloring corresponding to a consistent red/green/blue assignment.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Unary and Binary Constraints}
  ...
  \vspace{0.10in}
  \lectureplaceholder[1.55in]{Small visual with one variable under a unary restriction and two variables connected by a binary inequality constraint.}
\end{frame}
...
\begin{frame}[shrink=20]{Constraints Beyond ``Not Equal''}
  ...
  \vspace{0.12in}
  \lectureplaceholder[1.45in]{Three small application diagrams: precedence timeline, non-overlapping machine jobs, and resource-capacity scheduling.}
\end{frame}
...
\begin{frame}[shrink=20]{Why Structure Helps Search}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.55in]{Search tree where an early constraint violation prunes a large subtree of complete assignments.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Local Consistency: The Central Question}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.39\textwidth}
      \lectureplaceholder[2.55in]{Variable domains shown as sets of candidate values; inconsistent values are crossed out as constraints are considered.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Node Consistency}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.20in]{Domain {1,2,3,4} with 1 and 2 crossed out under the unary constraint X > 2.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Arc Consistency Example}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      \lectureplaceholder[2.60in]{Bipartite support diagram between X={1,2,3} and Y={2,3}; show valid X<Y support edges and cross out X=3.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Search Over Partial Assignments}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      \lectureplaceholder[2.65in]{Small backtracking tree for Australia: root {}, then WA assignments, then NT, with inconsistent branches crossed out.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Commutativity Removes Redundant Action Orders}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.41\textwidth}
      \lectureplaceholder[2.55in]{Two action sequences converging to the same set-valued partial assignment {WA=R, NT=G}; contrast with separate generic-search paths.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Worked Example: Beginning the Australia Search}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.70in]{Australia backtracking tree highlighting WA=R, then failed NT=R, followed by the next candidate NT=G.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Worked Example: Continuing After Failure}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.41\textwidth}
      \lectureplaceholder[2.65in]{Australia map with WA red, NT green, SA blue; alongside a search-tree fragment showing two rejected SA values and SA=blue accepted.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Completeness Does Not Mean Efficiency}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.55in]{Large backtracking tree with many explored branches; highlight that correctness does not prevent exponential growth.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Sudoku as a Constraint Satisfaction Problem}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      \lectureplaceholder[2.70in]{Partially filled 9x9 Sudoku grid; annotate one empty cell as variable $X_{r,c}$ with domain $\{1,\ldots,9\}$.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Sudoku Constraints}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.65in]{Sudoku grid with one row, one column, and one 3x3 box highlighted as three AllDifferent scopes.}
    \end{column}
  \end{columns}
