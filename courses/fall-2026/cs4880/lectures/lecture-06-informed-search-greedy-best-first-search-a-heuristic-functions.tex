...
\begin{frame}[shrink=20]{Last Time: Breadth-First Search}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.54\textwidth}
      \begin{block}{Unit-cost search}
        \begin{itemize}
          \item BFS expands states in increasing depth.
          \item With unit action costs, minimum depth is minimum path cost.
          \item A FIFO frontier is sufficient because every step contributes the same cost.
        \end{itemize}
      \end{block}

      \begin{itemize}
        \item In the source example, BFS reaches the goal using three edges.
        \item The search ignores edge-specific costs because the model treats every action equally.
      \end{itemize}
    \end{column}

    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.45in]{BFS table from A to Z with queue, visited set, and reconstructed path.}
    \end{column}
  \end{columns}

  \begin{keyidea}
    BFS is optimal only for the objective it represents: minimizing the number of unit-cost actions.
  \end{keyidea}
\end{frame}

\begin{frame}[shrink=20]{Last Time: Dijkstra / Uniform-Cost Search}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.54\textwidth}
      \begin{block}{Cost-aware frontier}
        \begin{itemize}
          \item Dijkstra / UCS expands the frontier state with minimum cumulative path cost.
          \item Priority is determined by $g(n)$.
          \item Edge relaxation replaces an expensive route when a cheaper route is discovered.
        \end{itemize}
      \end{block}

      \begin{itemize}
        \item The source example reaches $Z$ with total path cost $17$.
        \item Search is cost-aware, but it still has no estimate of which direction leads toward the goal.
      \end{itemize}
    \end{column}

    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.45in]{Dijkstra table from A to Z showing $g$ values, priority queue, and parent updates.}
    \end{column}
  \end{columns}

  \begin{keyidea}
    Uniform-Cost Search knows how expensive the path has been, but not how promising the remaining path appears.
  \end{keyidea}
\end{frame}
...
\begin{frame}[shrink=20]{Why Purely Greedy Search Can Be Misleading}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.55\textwidth}
      GBFS can perform well when the heuristic points reliably toward the goal.

      \begin{itemize}
        \item It may take long detours because it ignores $g(n)$.
        \item A locally attractive state can lead into an expensive region.
        \item Weak or deceptive heuristics can cause extensive exploration.
        \item GBFS does not provide a general minimum-cost guarantee.
      \end{itemize}

      \begin{block}{The missing information}
        Search needs both what we have already paid and what we estimate remains.
      \end{block}
    \end{column}

    \begin{column}{0.39\textwidth}
      \lectureplaceholder[2.48in]{Pathfinding scene where a greedy route is drawn toward the goal but must detour around obstacles.}
    \end{column}
  \end{columns}
\end{frame}
