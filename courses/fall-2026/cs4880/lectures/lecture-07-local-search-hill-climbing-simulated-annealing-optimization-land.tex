...
\begin{frame}[shrink=20]{From Path Finding to State Optimization}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.39\textwidth}
      \lectureplaceholder[2.55in]{Contrast a search tree/path with a single candidate configuration, such as a partially scrambled cube or optimization state.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{When Tree Search Fails}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.65in]{Exponentially expanding search tree beside a Rubik's Cube; label branching factor approximately 18.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Local Search}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.45in]{Single current-state node surrounded by neighboring states; arrows emphasize local movement rather than a stored tree.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{A New Problem: Eight Queens}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      \lectureplaceholder[2.65in]{Two 8-queens boards: one conflicted configuration and one valid non-attacking configuration.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Objective Function}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.45in]{Objective-function diagram for 8 queens showing attacking queen pairs and a conflict score.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Eight Queens as Optimization}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.44\textwidth}
      \lectureplaceholder[2.60in]{One 8-queens board with arrows showing a single queen moving within its column; annotate 56 possible neighbors.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{State-Space Landscape}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.41\textwidth}
      \lectureplaceholder[2.65in]{3D objective landscape labeled current state, shoulder, local maximum, flat local maximum, and global maximum.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Hill Climbing}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.41\textwidth}
      \lectureplaceholder[2.55in]{Objective surface with a trajectory climbing from a low point to a nearby peak.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Hill Climbing on Eight Queens}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.60in]{8-queens board over a contour landscape; show one queen move reducing the number of attacking pairs.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Why Hill Climbing Gets Stuck}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.41\textwidth}
      \lectureplaceholder[2.70in]{Landscape labeled local maximum, global optimum, ridge, plateau, shoulder, and flat local maximum.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Sideways Moves}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.55in]{Plateau landscape with a sideways trajectory, an eventual uphill exit, and a loop-risk annotation.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Stochastic Hill Climbing}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.41\textwidth}
      \lectureplaceholder[2.55in]{Landscape with several improving arrows and one randomly selected stochastic uphill move.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Best-First Hill Climbing}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.50in]{Current state with many possible neighbors; highlight the first sampled neighbor that improves the objective.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Random-Restart Hill Climbing}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.65in]{Multi-peak landscape with several random starting points and independent hill-climbing trajectories.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Simulated Annealing}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.42\textwidth}
      \lectureplaceholder[2.65in]{Annealing landscape showing an early trajectory that crosses out of a local optimum and later settles near a global optimum.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Temperature Controls Exploration}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.47\textwidth}
      ...
      \lectureplaceholder[1.40in]{Temperature-versus-time curves comparing fast and slow cooling schedules.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Local Beam Search}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.41\textwidth}
      \lectureplaceholder[2.60in]{Beam-search tree showing $k$ retained states at each level while other successors are discarded.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Beam Search Can Lose Diversity}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.46\textwidth}
      \lectureplaceholder[2.55in]{Two beam-search sketches: one collapsing to similar states and one maintaining diverse branches.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{The ``Hive Mind'' Intuition}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.46\textwidth}
      \lectureplaceholder[2.60in]{Four-stage infographic: crowd of candidates $\rightarrow$ fitness filter $\rightarrow$ recombination $\rightarrow$ random mutation spark.}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=20]{Genetic Algorithms}
  ...
  \vspace{0.10in}
  \lectureplaceholder[1.45in]{Genetic-algorithm cycle showing population $\rightarrow$ selection $\rightarrow$ crossover $\rightarrow$ mutation $\rightarrow$ next generation.}
...
\begin{frame}[shrink=20]{Genetic Algorithms on Eight Queens}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.75in]{8-queens genetic encoding with two parent vectors, crossover point, child vector, and a single-row mutation.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[shrink=20]{Is This AI?}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      \lectureplaceholder[2.55in]{Classical-AI local-search iconography beside modern generative-AI imagery; include optimization, robotics, and game examples.}
    \end{column}
  \end{columns}
